...
class HighFidelityGroupNormalization(nn.Module):
...
class AdaptiveChannelProcessor(nn.Module):
    def __init__(self, d_model: int, min_compression_ratio: int = 2, 
                 max_compression_ratio: int = 4):
        super().__init__()
    ...
    def forward(self, x):
        ...
        mixed_processed = (compression_weight.unsqueeze(1) * processed_features[0] + 
                          (1 - compression_weight.unsqueeze(1)) * processed_features[1])
...
class CARE(nn.Module):
    def __init__(self,
        ...
        self.norm1 = HighFidelityGroupNormalization(d_model, num_groups, preserve_ratio=preserve_ratio)
